...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    if (request.params.name === 'split') {
        const { fileContent, numObjects } = request.params.arguments;
        try {
            const jsonData = JSON.parse(fileContent);
            const keys = Object.keys(jsonData);
            if (numObjects <= 0 || numObjects > keys.length) {
                throw new Error(`Invalid 'numObjects' value. It should be between 1 and the number of top-level keys (${keys.length}).`);
            }
